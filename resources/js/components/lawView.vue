              ...
              <div class="container">
                  <div class="row"  v-if="f_parliament_sessions.length > 0">
                      ...
                      <div class="col-lg-3 mt-2" v-for="(f_parliament_session , fi)  in  f_parliament_sessions" :key="fi">
                        <button @click="session(f_parliament_session.law_type ,f_parliament_session.parliament_times_id , f_parliament_session.session_time_id)" class="btn col-12" type="button" id="background">
...
export default {
  ...
  methods: {
    ...
    session(lt , pid , st){
      ...
      window.location.href = '/law/show/'+lt+'/'+pid+'/'+st
  ...
  created() {
    let point = this;
    axios
    .post("/law/get/bypid", {'lawType' : point.lawType})
    .then(function (response) {
