<template>
         <div class="mb-6">
            <h5 class="mb-3">Display images <span v-if="this.requiredStatus == 1" class="text-danger">*</span></h5>
            <div>
                <input type="hidden" name="uploadsImage" :value="JSON.stringify(uploadImages)">
                <input :required="this.requiredStatus == 1 ? true : false"  type="file" @change="imagePreview($event)" class="form-control" name="images[]" multiple id="images" >
...
<script>
    export default {
        props : ['modelName' , 'requiredStatus'],
        data(){
            return {
               imageLists : [],
               uploadImages : [],
