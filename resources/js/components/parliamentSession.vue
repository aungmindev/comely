                                ...
                                <div :id="f_parliament_session.session_times.name" class="accordion-collapse collapse" aria-labelledby="headingThree" data-bs-parent="#accordionExample">
                                  <div class="accordion-body">
                                      <nav class="nav flex-column card">
                                          <a style="cursor : pointer" @click="session(f_parliament_session.sessiontype_id ,f_parliament_session.parliament_times_id , f_parliament_session.session_time_id , 1)" class="nav-link  border-bottom text-dark" aria-current="page">အစည်းအဝေးအစီအစဉ်များ <i class="fa-solid fa-photo-film float-end"></i></a>
                                          <a style="cursor : pointer" @click="session(f_parliament_session.sessiontype_id ,f_parliament_session.parliament_times_id , f_parliament_session.session_time_id , 0)" class="nav-link  border-bottom text-dark" >အစည်းအဝေးမှတ်တမ်းများ<i class="fa-solid fa-list float-end"></i></a>
                                        </nav>
                                  </div>
                                </div>
                              </div>
...
export default {
  ...
  methods: {
    ...
    session(sid , pid , st , sdt){
      // window.location.href = '/session/pid/{sessionTime?}/{sessionType?}'
      window.location.href = '/session/show/'+sid+'/'+pid+'/'+st+'/'+sdt
  ...
  created() {
    let point = this;
    axios
    .post("/session/get/bypid", {'stype' : point.sessionType})
    .then(function (response) {
